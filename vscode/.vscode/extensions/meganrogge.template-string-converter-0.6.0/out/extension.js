...
function activate(context) {
    let previousDocument = undefined;
    vscode.workspace.onDidChangeTextDocument((e) => __awaiter(this, void 0, void 0, function* () {
        ...
        const filesExcluded = configuration.get("template-string-converter.filesExcluded");
        if (enabled &&
            quoteType &&
            changes &&
            ...
            try {
                ...
                for (const selection of vscode.window.activeTextEditor.selections) {
                    ...
                    if (notAComment(lineText, currentChar, startQuoteIndex, endQuoteIndex) &&
                        ...
                        if (!!previousDocument) {
                            const current = getTemplateStringInfo(lineText, currentChar, lineNumber, e.document, convertWithinTemplateString !== null && convertWithinTemplateString !== void 0 ? convertWithinTemplateString : true);
                            ...
                            const usedToBeTemplateString = getTemplateStringInfo(previousDocument.lines[lineNumber].text, currentChar, lineNumber, previousDocument, convertWithinTemplateString !== null && convertWithinTemplateString !== void 0 ? convertWithinTemplateString : true).inTemplateString;
                            if (notTemplateStringWithinBackticks
                                ...
                                edit.replace(e.document.uri, new vscode.Range(backtickPositions.startBacktickPosition, backtickPositions.startBacktickPosition.translate(undefined, 1)), quoteType === 'single' ? '\'' : '"');
                                edit.replace(e.document.uri, new vscode.Range(backtickPositions.endBacktickPosition, backtickPositions.endBacktickPosition.translate(undefined, 1)), quoteType === 'single' ? '\'' : '"');
                        ...
                        else if (!getTemplateStringInfo(lineText, currentChar, lineNumber, e.document, convertWithinTemplateString !== null && convertWithinTemplateString !== void 0 ? convertWithinTemplateString : true).withinBackticks) {
...
function getTemplateStringInfo(line, currentCharIndex, cursorLine, document, convertWithinTemplateString) {
...
function includeFile(fileName, exclusions) {
